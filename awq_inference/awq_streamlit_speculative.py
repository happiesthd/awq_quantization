...
def load_vllm_resources():
    try:
        ...
        # --- IMPORTANT ---
        # Update this path to your AWQ quantized model directory
        model_id = "CPM_EXP_13.1_merged_awq"

        # Load the AWQ quantized model with vLLM
        llm = LLM(
            model=model_id,
            quantization="awq",                 # Specify that this is an AWQ model
            tensor_parallel_size=1,             # Set to your number of GPUs
            dtype='float16',
            gpu_memory_utilization=0.9,
            trust_remote_code=True,
            enforce_eager=True,
            # Speculative decoding is already correctly configured here
            speculative_config={
                "method": "ngram",
        ...
        # The tokenizer is needed for applying the chat template
...
def get_vllm_response(prompt, llm, tokenizer):
    try:
        ...
        # Using your desired sampling parameters to control output quality and repetition
        sampling_params = SamplingParams(
            max_tokens=2048,
            temperature=0.7,
            top_p=0.95,
            repetition_penalty=1.15, # Penalizes repeating words
            frequency_penalty=0.5,  # Penalizes repeating tokens based on their frequency
        )
...
if st.button("Generate Requirements"):
    if not user_input.strip():
        ...
        logger.info(f"Received input: {user_input[:100]}...") # Log first 100 chars
